...
allClients: any[] = [];
  ...
  ngOnInit(): void {
    ...
    this.clientFormGroup?.valueChanges?.subscribe(() => {
      ...
      const isInvalid =
        this.clientFormGroup.touched && this.clientFormGroup.invalid;
    ...
    this.addContact(); // initial row
    this.getById(); // Call API
  }
  ...
  getModuleAccess() {
    this.accessControlService
      ...
      .subscribe((res: any) => {
        // console.log(res);
        let temp = res.find(
          (item: any) => item.name === sessionStorage.getItem('access-name'),
        );
  ...
  public intialForm() {
    this.clientFormGroup = this.fb.group({
      ...
      allow_sending_status_report_to_client: [
        { value: false, disable: !this.shouldDisableFileds },
      ],
      allow_sending_public_holiday_information: [
        { value: false, disable: !this.shouldDisableFileds },
      ],
      practice_notes: [''],
    });
  ...
  public getClientUniqueNumber() {
    this.apiService
      .getData(
        `${environment.live_url}/${environment.clients}/?get-client-number=true`,
      )
      .subscribe(
        (respData: any) => {
          this.clientFormGroup.patchValue({
            client_number: respData?.client_unique_number,
          });
  ...
  public getCountryList() {
    this.allCountry = [];
    this.apiService
      .getData(`${environment.live_url}/${environment.settings_country}/`)
      .subscribe(
        (respData: any) => {
  ...
  public getSourceList() {
    this.apiService
      .getData(`${environment.live_url}/${environment.settings_source}/`)
      .subscribe(
        (respData: any) => {
          this.allSource = respData;
          console.log(respData, 'ressss');
        },
        (error: any) => {
          this.apiService.showError(error?.error?.detail);
        },
  ...
  filterEmployeeList(index: number): void {
    const search =
      this.searchEmployeeTextList[index]?.trim().toLowerCase() || '';
  ...
  filteredSourceList() {
    ...
    return this.allSource.filter((source: any) =>
      source?.source_name
        ?.toLowerCase()
        ?.includes(this.searchSourceText?.toLowerCase()),
    );
  }
  // Get Employee Detials
  public getClientDetails(id: any) {
    this.apiService
      .getData(`${environment.live_url}/${environment.clients}/${id}/`)
      .subscribe(
        (respData: any) => {
          this.clientFormGroup?.get('client_file')?.setErrors(null);
          if (respData?.client_file) {
            urlToFile(
              respData?.client_file,
              this.getFileName(respData?.client_file),
            )
              .then((file) => {
                this.file = file;
          ...
          this.clientFormGroup.patchValue({
            ...
            service_start_date: respData?.service_start_date
              ? new Date(respData?.service_start_date)?.toISOString()
              : null,
            service_end_date: respData?.service_end_date
              ? new Date(respData?.service_end_date)?.toISOString()
              : null,
            source: respData?.source_id,
            allow_sending_status_report_to_client:
              respData?.allow_sending_status_report_to_client,
            allow_sending_public_holiday_information:
              respData?.allow_sending_public_holiday_information,
          ...
          if (
            respData?.employee_details &&
            Array.isArray(respData?.employee_details) &&
            respData.employee_details?.length >= 1
          ) {
            this.employeeDetails = respData?.employee_details;
            const empDetailsArray = this.clientFormGroup.get(
              'employee_details',
            ) as FormArray;
            empDetailsArray.clear();
            respData?.employee_details?.forEach(
              (
                { employee, start_date, end_date, user__full_name }: any,
                index: any,
                array: any,
              ) => {
                ...
                this.dropdownState[`emp_${index}`].list =
                  this.selectedItemsMap[`emp_${index}`];
          ...
          if (
            respData?.contact_details &&
            Array.isArray(respData?.contact_details) &&
            respData.contact_details?.length >= 1
          ) {
            const contactDetailsArray = this.clientFormGroup.get(
              'contact_details',
            ) as FormArray;
            contactDetailsArray.clear();
            respData?.contact_details?.forEach(
              ({ name, email, phone_number }: any, index: any, array: any) => {
                const isLastItem = index === array?.length - 1;
                const contactForm = this.fb.group({
                  name: [{ value: name, disabled: !isLastItem }],
                  email: [{ value: email, disabled: !isLastItem }],
                  phone_number: [
                    { value: phone_number, disabled: !isLastItem },
                  ],
                });
  ...
  public addContact() {
    ...
    if (
      this.contactDetails?.at(lastItemIndex)?.valid &&
      this.contactDetails?.length < 5
    ) {
      const contact = this.contactDetails?.at(lastItemIndex);
      ['name', 'email', 'phone_number']?.forEach((field) =>
        contact?.get(field)?.disable(),
      );
    ...
    if (input.files && input.files?.length > 0) {
      ...
      if (
        ...
        this.apiService.showError(
          'Invalid file type. Only xlsx, xls, doc, docx, pdf files are allowed.',
        );
  ...
  private showConfirmationPopup(): Observable<boolean> {
    return new Observable<boolean>((observer) => {
      const modalRef = this.modalService.open(
        GenericRedirectionConfirmationComponent,
        {
          size: 'sm' as any,
          backdrop: true,
          centered: true,
  ...
  public deleteContent(id: any) {
    this.apiService
      .delete(`${environment.live_url}/${environment.clients}/${id}/`)
      .subscribe(
        async (data: any) => {
          if (data) {
            this.selectedClient = [];
  ...
  public saveClientDetails() {
    if (this.clientFormGroup.invalid) {
      ...
      if (this.isEditItem) {
        ...
        this.apiService
          .updateData(
            `${environment.live_url}/${environment.clients}/${this.client_id}/`,
            this.formData,
          )
          .subscribe(
            (respData: any) => {
        ...
        this.apiService
          .postData(
            `${environment.live_url}/${environment.clients}/`,
            this.formData,
          )
          .subscribe(
            (respData: any) => {
  ...
  public createFromData() {
    ...
    let updatedStartDateValue = this.datepipe.transform(
      this.clientFormGroup?.get('service_start_date')?.value,
      'yyyy-MM-dd',
    );
    let updatedEndDateValue = this.datepipe.transform(
      this.clientFormGroup?.get('service_end_date')?.value,
      'yyyy-MM-dd',
    );
    this.formData.set(
      'client_number',
      this.clientFormGroup?.get('client_number')?.value,
    );
    this.formData.set(
      'client_name',
      this.clientFormGroup?.get('client_name')?.value,
    );
    ...
    this.formData.set(
      'practice_notes',
      this.clientFormGroup?.get('practice_notes')?.value || '',
    );
    this.formData.set(
      'allow_sending_status_report_to_client',
      this.clientFormGroup?.get('allow_sending_status_report_to_client')?.value
        ? 'true'
        : 'false',
    );
    this.formData.set(
      'allow_sending_public_holiday_information',
      this.clientFormGroup?.get('allow_sending_public_holiday_information')
        ?.value
        ? 'true'
        : 'false',
    );
    const result = this.clientFormGroup
      ?.get('contact_details')
      ?.getRawValue()
      .map((item: any) => {
        return {
    ...
    const empresult = this.clientFormGroup
  ...
  addEmployee() {
    if (this.employeeFormArray.length >= 1) {
      ...
      if (this.employeeFormArray?.at(lastItemIndex)?.valid) {
        ...
        ['employee_id', 'start_date', 'end_date'].forEach((field) =>
          empControls?.get(field)?.disable(),
        );
  ...
  removeEmployee(index: number) {
    if (this.employeeFormArray?.length > 1) {
      ...
      this.employeeFormArray.controls.forEach((_, i) => {
        newDropdownState[`emp_${i}`] =
          this.dropdownState[`emp_${i >= index ? i + 1 : i}`] ||
          this.initEmployeeDropdownState(i);
        newSelectedItemsMap[`emp_${i}`] =
          this.selectedItemsMap[`emp_${i >= index ? i + 1 : i}`] || [];
      });
  ...
  public onEmployeeChange(event: any, i: any) {
    ...
    const isEmployeeDuplicate = formArray.some(
  ...
  getEmployeeName(userId: number): string {
    const employee = this.allEmployeeList?.find(
      (emp: any) => emp?.user_id === userId,
    );
    ...
    for (let i = 0; i < formArray?.controls?.length; i++) {
      ...
      if (email && emailset.has(email)) {
        ...
        if (emailErrors['duplicateEmail']) {
          ...
          emailControl?.setErrors(
            Object.keys(emailErrors)?.length ? emailErrors : null,
          );
        }
      }
      ...
      if (value && phoneset.has(value)) {
        ...
        if (phoneErrors['duplicatePhoneNo']) {
          ...
          phoneNumberControl?.setErrors(
            Object.keys(phoneErrors)?.length ? phoneErrors : null,
          );
        }
      }
    }
  ...
  canDeactivate(): Observable<boolean> {
    ...
    const isFormChanged: boolean =
      JSON.stringify(currentFormValue) !==
      JSON.stringify(this.initialFormValue);
    return this.formErrorScrollService.isFormDirtyOrInvalidCheck(
      isFormChanged,
  ...
  shouldShowAddButton(data: any): boolean {
    ...
    const matchedDetail: any = this.employeeDetails.find(
      (emp: any) => emp.employee === currentEmployee,
    );
  ...
  fetchData(key: string, append = false) {
    ...
    this.apiService
      .getData(
        `${environment.live_url}/${this.dropdownEndpoints.emp}/?${query}`,
      )
      .subscribe(
        (res: any) => {
  ...
  updateSelectedItems(key: string, selectedId: any) {
    ...
    if (!selectedItems.some((item) => item.user_id === selectedId)) {
      const found = state.list.find((item: any) => item.user_id === selectedId);
      if (found) {
  ...
  //     phone_number: ['', Validators.required],
  //     isSelected: [false]   // ✅ Checkbox control added
  //   });
  ...
  createContactGroup(data: any): FormGroup {
    return this.fb.group({
...
getById() {
  const response = [
    ...
    {
      client_id: 103,
      client_name: 'Global Tech Systems',
      contact_details: [
        {
          name: 'Arjun',
        ...
        {
          name: 'Suresh',
          email: 'suresh@globaltech.com',
          phone_number: '7755332211',
          isSelected: true,
          rowSelected: false
        }
      ]
    ...
    {
      client_id: 104,
      client_name: 'Sunrise Enterprises',
      contact_details: [
        {
          name: 'Priya',
    ...
    {
      client_id: 107,
      client_name: 'Elite Softwares',
      contact_details: [
        {
          name: 'Deepak',
          email: 'deepak@elite.com',
...
patchContactDetails(data: any[]) {
  this.contactDetails.clear();

  data.forEach((client) => {
    client.contact_details.forEach((contact: any) => {
      ...
      const formGroup = this.fb.group({
        ...
        rowSelected: [contact.rowSelected || false],
        isSelected: [contact.isSelected || false],
      });

      this.contactDetails.push(formGroup);
    });
  });
}


getFormGroup(clientId: number, email: string): FormGroup {
  return this.contactDetails.controls.find((control: any) =>
    control.value.client_id === clientId &&
    control.value.email === email
  ) as FormGroup;
}
sendRequest() {
  ...
  const selectedRows = this.contactDetails.value.filter(
    (contact: any) => contact.rowSelected === true
  );

  selectedRows.forEach((contact: any) => {

    if (!groupedData[contact.client_id]) {
      groupedData[contact.client_id] = {
        client_id: contact.client_id,
        client_contact_deatils: []
      };
    }

    groupedData[contact.client_id].client_contact_deatils.push({
      email: contact.email,
      is_sending_feedback_form_enabled: contact.isSelected
    });
  ...
  selectAllRows(checked: boolean) {
    this.contactDetails.controls.forEach((control) => {
      control.get('rowSelected')?.setValue(checked);
    });
  }
  ...
  isAllRowsSelected(): boolean {
    return (
      ...
      this.contactDetails.controls.every(
        (control) => control.get('rowSelected')?.value === true,
      )
    );
  }

  isRowsIndeterminate(): boolean {
    const selectedCount = this.contactDetails.controls.filter(
      (control) => control.get('rowSelected')?.value === true,
    ).length;

    return selectedCount > 0 && selectedCount < this.contactDetails.length;
  }
}
