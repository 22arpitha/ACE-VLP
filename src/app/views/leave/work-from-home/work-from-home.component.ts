...
export class WorkFromHomeComponent implements OnInit {
  ...
  //   availableBalance: number = 5;

  //   readonly WFHCategory = WFHCategory;
  //   readonly maxReasonLength = 250;

  //   categories = [
  //     {
  //       value: WFHCategory.LIMITED_FLEXIBILITY,
  //       label: 'Limited Flexibility',
  //       description: '5 days per quarter for flexible work arrangements',
  //       icon: 'home_work'
  //     },
  //     {
  //       value: WFHCategory.PROLONGED_HEALTH_ISSUES,
  //       label: 'Prolonged Health Issues',
  //       description: 'For medical reasons requiring extended work from home',
  ...
  //       category: [WFHCategory.LIMITED_FLEXIBILITY, Validators.required],
  //       fromDate: ['', Validators.required],
  //       toDate: ['', Validators.required],
  //       reason: ['', [Validators.required, Validators.maxLength(this.maxReasonLength)]],
  //       attachment: ['']
  //     });
  //   }
  ...
  //     this.applicationForm.get('category')?.valueChanges
  //       .pipe(takeUntil(this.destroy$))
  //       .subscribe(category => {
  //         this.onCategoryChange(category);
  //       });

  //     // Listen to date changes with debounce
  //     const fromDateControl = this.applicationForm.get('fromDate');
  //     const toDateControl = this.applicationForm.get('toDate');
  ...
  //     const currentQuarter = this.wfhService.getCurrentQuarter();
  //     this.wfhService.getBalance(
  //       'EMP001', // Replace with actual employee ID
  //       WFHCategory.LIMITED_FLEXIBILITY,
  //       currentQuarter.quarter,
  //       currentQuarter.year
  //     ).pipe(takeUntil(this.destroy$))
  //     .subscribe((balance: { availableDays: number; }) => {
  //       this.availableBalance = balance.availableDays;
  //     });
  //   }

  //   onCategoryChange(category: WFHCategory): void {
  //     const attachmentControl = this.applicationForm.get('attachment');

  //     if (category === WFHCategory.PROLONGED_HEALTH_ISSUES) {
  //       attachmentControl?.setValidators([Validators.required]);
  //       this.toastr.info('Medical certificate is mandatory for health issues', 'Info');
  //     } else {
  ...
  //     // Validate date range
  //     if (to < from) {
  //       this.toastr.error('End date must be after start date', 'Invalid Date Range');
  //       this.weekDays = [];
  //       this.workingDaysCount = 0;
  //       return;
  //     }
  ...
  //     if (from < today) {
  //       this.toastr.warning('Start date should not be in the past', 'Warning');
  //     }

  //     this.isCalculating = true;

  //     this.wfhService.calculateWorkingDays(from, to)
  //       .pipe(takeUntil(this.destroy$))
  //       .subscribe({
  //         next: (days: WeekDay[]) => {
  //           this.weekDays = days;
  //           this.workingDaysCount = this.wfhService.getWorkingDaysCount(days);
  //           this.isCalculating = false;

  //           // Validate balance for limited flexibility
  //           if (this.applicationForm.get('category')?.value === WFHCategory.LIMITED_FLEXIBILITY) {
  //             if (this.workingDaysCount > this.availableBalance) {
  //               this.toastr.error(
  //                 `Insufficient balance. Available: ${this.availableBalance} days, Required: ${this.workingDaysCount} days`,
  ...
  //         error: () => {
  //           this.isCalculating = false;
  //           this.toastr.error('Failed to calculate working days', 'Error');
  //         }
  //       });
  //   }
  ...
  //         this.toastr.error('Only JPG, PNG, and PDF files are allowed', 'Invalid File Type');
  //         event.target.value = '';
  //         return;
  //       }

  //       this.selectedFile = file;
  //       this.applicationForm.patchValue({ attachment: file.name });
  ...
  //     this.applicationForm.patchValue({ attachment: '' });
  //   }
  ...
  //       this.toastr.error('Please select valid working days', 'No Working Days');
  //       return;
  //     }
  ...
  //     // Validate balance for limited flexibility
  //     if (category === WFHCategory.LIMITED_FLEXIBILITY) {
  //       if (this.workingDaysCount > this.availableBalance) {
  //         this.toastr.error(
  //           `Insufficient balance. Available: ${this.availableBalance} days`,
  //           'Insufficient Balance'
  //         );
  //         return;
  //       }
  ...
  //     // Validate attachment for health issues
  //     if (category === WFHCategory.PROLONGED_HEALTH_ISSUES && !this.selectedFile) {
  //       this.toastr.error('Medical certificate is required for health issues', 'Missing Attachment');
  //       return;
  //     }
  ...
  //     const application: Partial<WFHApplication> = {
  //       employeeId: 'EMP001', // Replace with actual employee ID
  //       employeeName: 'John Doe', // Replace with actual employee name
  //       managerId: 'MGR001', // Replace with actual manager ID
  //       managerName: 'Jane Smith', // Replace with actual manager name
  //       directorId: 'DIR001', // Required for health issues
  //       directorName: 'Robert Johnson',
  //       category: formValue.category,
  //       fromDate: new Date(formValue.fromDate),
  //       toDate: new Date(formValue.toDate),
  //       numberOfDays: this.workingDaysCount,
  //       weekDays: this.weekDays,
  //       reason: formValue.reason,
  //       attachmentName: this.selectedFile?.name,
  //       attachmentUrl: this.selectedFile ? 'mock-url/' + this.selectedFile.name : undefined
  //     };

  //     this.wfhService.submitApplication(application)
  //       .pipe(takeUntil(this.destroy$))
  //       .subscribe({
  //         next: (result: { id: any; }) => {
  //           this.toastr.success(
  //             `Application submitted successfully. ID: ${result.id}`,
  //             'Success',
  //             { timeOut: 5000 }
  //           );
  //           this.resetForm();
  ...
  //           this.toastr.error(error.message || 'Failed to submit application', 'Error');
  //           this.isSubmitting = false;
  //         }
  //       });
  //   }
  ...
  //       category: WFHCategory.LIMITED_FLEXIBILITY
  //     });
  //     this.weekDays = [];
  //     this.workingDaysCount = 0;
  //     this.selectedFile = null;
  ...
  //   get categoryControl() {
  //     return this.applicationForm.get('category');
  //   }

  //   get fromDateControl() {
  //     return this.applicationForm.get('fromDate');
  //   }

  //   get toDateControl() {
  //     return this.applicationForm.get('toDate');
  //   }
  ...
  //     return this.applicationForm.get('attachment');
  //   }

  //   get remainingChars(): number {
  //     const reason = this.reasonControl?.value || '';
  //     return this.maxReasonLength - reason.length;
  //   }
  ...
  BreadCrumbsTitle: any = 'Apply for Work From Home';
  sessions: any = [
  ...
  workCalendar: any;
  wfhBalance = 0;
  showWfhBalance: boolean = false;
   wfhCategories: any[] = [];
  genders = [
    { name: 'Male', value: '1', value_check: 'male' },
    { name: 'Female', value: '2', value_check: 'female' },
    { name: 'Others', value: '3', value_check: 'others' },
  ];
  ...
  ngOnInit(): void {
    ...
    this.leaveApplyForm
      ...
      ?.valueChanges.subscribe((value: any) => {
        ...
        if (value !== 29) {
          this.showWfhBalance = false;
        } else {
          this.showWfhBalance = true;
        }
      });
  ...
  getUserData() {
    this.apiService.getUserById(this.user_id).subscribe((res: any) => {
      this.employeeGender = this.genders.find(
        (x: any) => x.value == res?.user__gender,
      );
      ...
      this.leaveApplyForm.patchValue({
        reporting_to: this.reportinManagerDetails[0]?.id,
      });
    });
  }
  ...
  onLeaveTypeChange(event: any) {
    let temp = this.allleavetypeList.find(
      (item: any) => item.leave_type_id === event.value,
    );
    ...
    if (
      this.selectedLeaveTypeName.includes('paternity') ||
      this.selectedLeaveTypeName.includes('maternity')
    ) {
      this.leaveApplyForm.controls['supporting_document'].setValidators([
        Validators.required,
      ]);
    } else {
      this.leaveApplyForm.controls['supporting_document'].setValidators(null);
      this.leaveApplyForm.controls['supporting_document'].setErrors(null);
    }
    this.leaveApplyForm.controls['supporting_document'].updateValueAndValidity();
  }

  initialForm() {
    this.leaveApplyForm = this.fb.group({
      wfh_type: ['', Validators.required],
    ...
    this.conditionalAttachmentValidation();
  }
  ...
  conditionalAttachmentValidation() {
   
    const leaveTypeCtrl = this.leaveApplyForm.get('wfh_type');
   

  
    leaveTypeCtrl?.valueChanges.subscribe(() => {
  ...
  updateAttachmentValidation() {
    const attachmentCtrl = this.leaveApplyForm.get('supporting_document');
    const leaveType = this.leaveApplyForm.get('wfh_type')?.value;

    // const isRequired = leaveType === 'prolonged_health_issue';
     const isRequired = leaveType === 30;
    ...
    attachmentCtrl?.updateValueAndValidity();
  }
  ...
  public getAllLeaveTypes() {
    ...
    this.apiService
      .getData(
        `${environment.live_url}/${environment.wfh_type_conf}/?employee=${this.user_id}`,
      )
      .subscribe(
        (respData: any) => {
          ...
          //   (item: any) =>
          //     item.is_active === true &&
          //     (item.leave_for === this.employeeGender?.value_check ||
          //       item.leave_for === 'all employees'),
          // );
        },
        (error: any) => {
  ...
  onSubmit(): void {
    ...
    let new_start_date: any = this.convertDateTime(
      this.getDateFromControl('from_date'),
    );
    let new_end_date: any = this.convertDateTime(
      this.getDateFromControl('to_date'),
    );
    // prepare cc and file
    this.leaveApplyForm.patchValue({
    ...
    if (this.selectedFile) {
      this.leaveApplyForm.patchValue({ supporting_document: this.selectedFile });
    }
    ...
    const leaveTypeData = this.allleavetypeList?.find(
    ...
    if (fromDate < current) {
      if (
        leaveTypeData?.utilization_after &&
        Number(leaveTypeData.utilization_after) > 0
      ) {
        if (toDate < current) {
          // Count days from current date to TO date, excluding TO date
          const daysDiff = Math.floor(
            (current.getTime() - toDate.getTime()) / msPerDay,
          );
    ...
    if (fromDate >= current) {
      if (
        leaveTypeData?.utilization_before &&
        Number(leaveTypeData.utilization_before) > 0
      ) {
        const daysDiff = Math.floor(
          (fromDate.getTime() - current.getTime()) / msPerDay,
        );
    ...
    this.apiService
      .postData(
        `${environment.live_url}/${environment.apply_wfh}/`,
        formData,
      )
      .subscribe(
        (res: any) => {
  ...
  getWfhBalance() {
    this.apiService
      ...
      .subscribe(
        (res: any) => {
          this.wfhBalance = res.closing_balance;
          console.log(res, 'wfh balance');
        },
        (error: any) => {
          console.error(error);
        },
