...
import { tableColumns } from '../wfh-prolonged-health-issues-summary-report/wfh-prolonged-health-issues-summary-report.config';
...
@Component({
  ...
  templateUrl: './wfh-prolonged-health-issues-summary-report.component.html',
  styleUrls: ['./wfh-prolonged-health-issues-summary-report.component.scss'],
  standalone:false
})
export class WfhProlongedHealthIssuesSummaryReportComponent implements OnInit {

 BreadCrumbsTitle: any = 'WFH Proloned Health Issues Summary Report';
   term: string = '';
   tableSize: number = 50;
   page: any = 1;
   ...
   exportCsvOrPdf(fileType:string) {
     ...
     const url = `${environment.live_url}/${environment.wfh_prolonged_health_summary_report}/${query}`;
     // console.log(url);
  ...
  //    }
  //    await this.api.getData(`${environment.live_url}/${environment.wfh_prolonged_health_summary_report}/${finalQuery}`).subscribe((res: any) => {
  //      if (res.results) {
  ...
  this.api.getData(
    `${environment.live_url}/${environment.wfh_prolonged_health_summary_report}/${finalQuery}`
  ).subscribe((res: any) => {
