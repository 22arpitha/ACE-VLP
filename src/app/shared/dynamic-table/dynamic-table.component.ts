...
export class DynamicTableComponent implements OnInit, OnChanges, DoCheck {
  ...
  getEmployeesList(reset: boolean) {
    ...
    this.api
      .getData(`${environment.live_url}/${environment.user}/${query}`)
      .subscribe(
        (respData: any) => {
  ...
  private initializeTable(): void {
    ...
    this.config.columns?.forEach((col) => {
      ...
      if (this.directionValue) {
        this.arrowState[this.sortValue] =
          this.directionValue === 'ascending' ? true : false;
      } else {
        this.arrowState[col.sortKey] = false;
      }
      ...
      const matchSearch =
        ...
        this.config.columns?.some((col) =>
          row[col.key]
            ?.toString()
            ?.toLowerCase()
            ?.includes(this.config.searchTerm!?.toLowerCase())
        );
  ...
  private updatePagination(): void {
    ...
    const pageSize =
      this.config?.tableSize && this.config.tableSize > 0
        ? this.config.tableSize
        : 50;
  ...
  requestReset() {
    ...
    this.selectedDate = '';
    this.resetWeekDate = true;
    (this.mainStartDate = ''), (this.mainEndDate = '');
    this.tempFilters = {};
    this.columnFilters = {};
    this.arrowState = {};
    this.sortValue = '';
  ...
  setDateFilterColumn(columnKey: string): void {
    ...
    this.dateFilterValue = this.columnFilters[columnKey]
      ? new Date(this.columnFilters[columnKey])
      : null;
  }
  ...
  onDateSelected(event: any): void {
    ...
    if (this.activeDateColumn) {
      const formattedDate = selectedValue
        ? this.datePipe.transform(selectedValue, 'yyyy-MM-dd')
        : null;
      ...
      this.actionEvent.emit({
        actionType: 'dateFilter',
        detail: formattedDate,
        key: this.activeDateColumn,
      });
  ...
  public async submitWorkCultureDetails() {
    if (this.rows.valid) {
      ...
      await this.UpdateFileFieldData(workCultureData)
        .then((updatedData) => {
  ...
  get estimatedTotal(): number {
    return this.paginatedData.reduce(
      (sum, item) => sum + parseFloat(item.estimatedTime),
      0
    );
  }
  ...
  get actualTotal(): number {
    return this.paginatedData.reduce(
      (sum, item) => sum + parseFloat(item.actualTime),
      0
    );
  }
  ...
  get tooltipMessage(): string | null {
    if (!this.allow_sending_status)
      return 'Allow sending the status report to client is not enabled.';
  ...
  buildDynamicTableForm(tableData) {
    ...
    tableData?.forEach((item, index) => {
      ...
      if (item && item?.work_ethics_file) {
        urlToFile(
          item?.work_ethics_file,
          this.getFileName(item?.work_ethics_file)
        )
          .then((file) => {
            if (file) {
    ...
    if (workCulData && workCulData?.length >= 1) {
      ...
      for (let index = 0; index < workCulData?.length; index++) {
        ...
        if (this.file && this.file[index]) {
          workCulData[index].work_ethics_file = await this.convertFileToBase64(
            this.file[index]
          );
  ...
  mainEndDateChange(event: any) {
    if (event.value) {
      ...
      this.actionEvent.emit({
        actionType: 'mainDateRangeFilter',
        detail: { startDate: this.dateRangeStartDate, endDate: formattedDate },
      });
      this.resetWeekDate = true;
    }
  }
  ...
  onFilterOpen(col: any, isOpen: boolean) {
    if (isOpen) {
      ...
      if (
        ...
        columnConfig.filterOptions.length > 0
      ) {
        return;
      }
  ...
  fetchData(key: string, append = false) {
    ...
    this.api
      .getData(
        `${environment.live_url}/${this.dropdownEndpoints[key]}/?${query}`
      )
      .subscribe(
        (res: any) => {
